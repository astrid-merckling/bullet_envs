...
class WalkerBase(MJCFBasedRobot):
  ...
  def __init__(self, fn, robot_name, action_dim, obs_dim, power,randomExplor=True):
    MJCFBasedRobot.__init__(self, fn, robot_name, action_dim, obs_dim,randomExplor=randomExplor)
    self.power = power
    self.camera_x = 0
  ...
  def robot_specific_reset(self, bullet_client):
    ...
    if self.randomExplor:
      for j in self.ordered_joints:
        j.reset_current_position(self.np_random.uniform(low=-0.1, high=0.1), 0)# (low=-0.1, high=0.1)
    # else:
    #   self.parts[self.foot_list[1]].reset_position(np.array((0,0,-0.1)))
  ...
  def apply_action(self, a):
    assert (np.isfinite(a).all()), 'infinite: %s'%a
  ...
  def calc_state(self):
    ...
    obs = np.concatenate([more] + [j])
    return obs.astype(np.float32)


  def measure_robot(self):
    current_x = self.parts['torso'].pose().xyz()[0]

    measure = np.array([np.hstack((p.pose().xyz()[0]-current_x,p.pose().xyz()[2],p.pose().rpy()[1])) for k ,p in self.parts.items() if p.bodyPartIndex != -1 and 'link0' not in k]) # remove floor
...
class Hopper(WalkerBase):
  ...
  def __init__(self,randomExplor=True):
    WalkerBase.__init__(self, "hopper.xml", "torso", action_dim=3, obs_dim=15, power=0.75,randomExplor=randomExplor)

  def alive_bonus(self, z, pitch):
    # return +1 if z > 0.7 and abs(pitch) < .2 else -1
    return +1 if z > 0.6 and abs(pitch) < .2 else -1
...
class Walker2D(WalkerBase):
  ...
  def __init__(self,randomExplor=True):
    WalkerBase.__init__(self, "walker2d.xml", "torso", action_dim=6, obs_dim=22, power=0.40,randomExplor=randomExplor)
    self.healthy_z_range=(0.7, 2.0)#(0.8, 2.0)

  def alive_bonus(self, z, pitch):
    min_z, max_z = self.healthy_z_range
    return +1 if min_z < z < max_z and abs(pitch) < 1.0 else -1
...
class HalfCheetah(WalkerBase):
  ...
  def __init__(self,randomExplor=True):
    WalkerBase.__init__(self, "half_cheetah.xml", "torso", action_dim=6, obs_dim=18, power=0.90,randomExplor=randomExplor)
    self.healthy_z_range = (0.3, 2.0)

  def alive_bonus(self, z, pitch):
    # Use contact other than feet to terminate episode: due to a lot of strange walks using knees
    min_z, max_z = self.healthy_z_range
    return +1 if min_z < z < max_z else -1
    # return +0.2 if np.abs(pitch) < 1.0 and not self.feet_contact[1] and not self.feet_contact[
    #     2] and not self.feet_contact[4] and not self.feet_contact[5] else -0.2
...
class Ant(WalkerBase):
  ...
  def __init__(self,randomExplor=True):
    WalkerBase.__init__(self, "ant.xml", "torso", action_dim=8, obs_dim=28, power=2.5,randomExplor=randomExplor)
...
class Humanoid(WalkerBase):
  ...
  def __init__(self,randomExplor=True):
    WalkerBase.__init__(self,'humanoid_symmetric.xml','torso',action_dim=17,obs_dim=44,power=0.41,randomExplor=randomExplor)
...
class HumanoidFlagrun(Humanoid):
  ...
  def __init__(self,randomExplor=True):
    Humanoid.__init__(self,randomExplor=randomExplor)
    self.flag = None
...
class HumanoidFlagrunHarder(HumanoidFlagrun):
  ...
  def __init__(self,randomExplor=True):
    HumanoidFlagrun.__init__(self,randomExplor=randomExplor)
    self.flag = None
