...
class WalkerBase(MJCFBasedRobot):
    def __init__(self, fn, robot_name, action_dim, obs_dim, power, randomExplor=True):
        MJCFBasedRobot.__init__(
            self, fn, robot_name, action_dim, obs_dim, randomExplor=randomExplor
        )
        self.power = power
        self.camera_x = 0
    ...
    def measure_robot(self):
        ...
        measure = np.array(
            [
                np.hstack(
                    (
                        p.pose().xyz()[0] - current_x,
                        p.pose().xyz()[2],
                        p.pose().rpy()[1],
                    )
                )
                for k, p in self.parts.items()
                if p.bodyPartIndex != -1 and "link0" not in k
            ]
        )  # remove floor
...
class Hopper(WalkerBase):
    ...
    def alive_bonus(self, z, pitch):
        ...
        return +1 if z > 0.6 and abs(pitch) < 0.2 else -1
