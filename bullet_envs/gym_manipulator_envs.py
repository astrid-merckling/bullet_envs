...
class ReacherBulletEnv(MJCFBaseBulletEnv):
    ...
    def step(self, a):
        ...
        reward = -(
            self.goal_distance(self.robot.calc_object(), self.robot.calc_target())
            > self.target_radius
        ).astype(np.float32)
...
class StrikerBulletEnv(MJCFBaseBulletEnv):
    ...
    def step(self, a):
        ...
        dist_object_finger = (
            self.robot.object.pose().xyz() - self.robot.fingertip.pose().xyz()
        )
        reward_dist_vec = (
            self.robot.object.pose().xyz() - self.robot.target.pose().xyz()
        )  # TODO: Should the object and target really belong to the robot? Maybe split this off

        self._min_strike_dist = min(
            self._min_strike_dist, np.linalg.norm(reward_dist_vec)
        )
...
class ThrowerBulletEnv(MJCFBaseBulletEnv):
    ...
    def step(self, a):
        ...
        if (
            not self.robot._object_hit_ground
            and self.robot.object.pose().xyz()[2] < -0.25
        ):  # TODO: Should the object and target really belong to the robot? Maybe split this off
