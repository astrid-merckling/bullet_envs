        ...
        if self.doneLoading == 0:
            ...
            if self.self_collision:
                ...
                self.objects = self._p.loadMJCF(
                    os.path.join(
                        bullet_envs_path, "pybullet_data", "mjcf", self.model_xml
                    ),
                    flags=pybullet.URDF_USE_SELF_COLLISION
                    | pybullet.URDF_USE_SELF_COLLISION_EXCLUDE_ALL_PARENTS,
                )
            ...
            if "reacher" in self.model_xml:
                ...
                self._p.changeVisualShape(
                    self.parts["root"].bodyIndex,
                    self.parts["sideS"].bodyPartIndex,
                    rgbaColor=[0.3, 0.3, 0.3, 1],
                )
                self._p.changeVisualShape(
                    self.parts["sideE"].bodyIndex,
                    self.parts["sideE"].bodyPartIndex,
                    rgbaColor=[0.3, 0.3, 0.3, 1],
                )
                self._p.changeVisualShape(
                    self.parts["sideN"].bodyIndex,
                    self.parts["sideN"].bodyPartIndex,
                    rgbaColor=[0.3, 0.3, 0.3, 1],
                )
                self._p.changeVisualShape(
                    self.parts["sideW"].bodyIndex,
                    self.parts["sideW"].bodyPartIndex,
                    rgbaColor=[0.3, 0.3, 0.3, 1],
                )
                # bug with 'root' -> 'body0'
                self._p.changeVisualShape(
                    self.parts["body0"].bodyIndex,
                    self.parts["root"].bodyPartIndex,
                    rgbaColor=[0.75, 0.88, 0.92, 1],
                )
                # bug with 'body0' -> 'target'
                self._p.changeVisualShape(
                    self.parts["target"].bodyIndex,
                    self.parts["body0"].bodyPartIndex,
                    rgbaColor=[0.81, 0.61, 0.49, 1],
                )
                # bug with 'body1' -> 'target'
                self._p.changeVisualShape(
                    self.parts["target"].bodyIndex,
                    self.parts["body1"].bodyPartIndex,
                    rgbaColor=[0.81, 0.61, 0.49, 1],
                )
                # bug with 'fingertip' -> 'target'
                self._p.changeVisualShape(
                    self.parts["target"].bodyIndex,
                    self.parts["fingertip"].bodyPartIndex,
                    rgbaColor=[0.47, 0.35, 0.27, 1],
                )
                # ball
