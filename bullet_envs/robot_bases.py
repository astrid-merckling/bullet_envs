...
class MJCFBasedRobot(XmlBasedRobot):
  ...
  def __init__(self, model_xml, robot_name, action_dim, obs_dim, self_collision=True,randomExplor=True,display_target=False):
  ...
  def reset(self, bullet_client):
    ...
    if (self.doneLoading == 0):
      ...
      if self.self_collision:
        ...
        self.objects = self._p.loadMJCF(os.path.join(bullet_envs_path,"pybullet_data", "mjcf", self.model_xml),
                                        flags=pybullet.URDF_USE_SELF_COLLISION |
                                              pybullet.URDF_USE_SELF_COLLISION_EXCLUDE_ALL_PARENTS)
        ...
        self.objects = self._p.loadMJCF(os.path.join(pybullet_data.getDataPath(), "mjcf", self.model_xml))
        self.parts, self.jdict, self.ordered_joints, self.robot_body = self.addToScene(self._p, self.objects)
      if 'reacher' in self.model_xml:

        # texUid = self._p.loadTexture(os.path.join(bullet_envs_path,"pybullet_data", "assets", "square.png"))
        # self._p.changeVisualShape(self.parts['ground'].bodyIndex, self.parts['ground'].bodyPartIndex,
        #                           textureUniqueId=texUid)  # to modify ground
        # bug with 'sideS' -> 'root'
        self._p.changeVisualShape(self.parts['root'].bodyIndex, self.parts['sideS'].bodyPartIndex,
                                  rgbaColor=[.3, .3, .3, 1])
        self._p.changeVisualShape(self.parts['sideE'].bodyIndex, self.parts['sideE'].bodyPartIndex,
                                  rgbaColor=[.3, .3, .3, 1])
        self._p.changeVisualShape(self.parts['sideN'].bodyIndex, self.parts['sideN'].bodyPartIndex,
                                  rgbaColor=[.3, .3, .3, 1])
        self._p.changeVisualShape(self.parts['sideW'].bodyIndex, self.parts['sideW'].bodyPartIndex,
                                  rgbaColor=[.3, .3, .3, 1])
        # bug with 'root' -> 'body0'
        self._p.changeVisualShape(self.parts['body0'].bodyIndex, self.parts['root'].bodyPartIndex,
                                  rgbaColor=[0.75, .88, .92, 1])
        # bug with 'body0' -> 'target'
        self._p.changeVisualShape(self.parts['target'].bodyIndex, self.parts['body0'].bodyPartIndex,
                                  rgbaColor=[0.81, .61, .49, 1])
        # bug with 'body1' -> 'target'
        self._p.changeVisualShape(self.parts['target'].bodyIndex, self.parts['body1'].bodyPartIndex,
                                  rgbaColor=[0.81, .61, .49, 1])
        # bug with 'fingertip' -> 'target'
        self._p.changeVisualShape(self.parts['target'].bodyIndex,
                                  self.parts['fingertip'].bodyPartIndex, rgbaColor=[0.47, .35, .27, 1])
        # ball
        if self.display_target:
          self._p.changeVisualShape(8, 2, rgbaColor=[1, 0.3, 0.3, 1])
        else:
          self._p.changeVisualShape(8, 2, rgbaColor=[1, 1, 0.3, 0])
