...
class InvertedPendulum(MJCFBasedRobot):
  ...
  def __init__(self,randomExplor=True):
    MJCFBasedRobot.__init__(self, 'inverted_pendulum.xml', 'cart', action_dim=1, obs_dim=5,randomExplor=randomExplor)
  ...
  def robot_specific_reset(self, bullet_client):
    ...
    if self.randomExplor:
      u = self.np_random.uniform(low=-.3, high=.3) #(low=-0.1, high=0.1)
      self.j1.reset_current_position(u if not self.swingup else 3.1415 + u, 0)
      u = self.np_random.uniform(low=-.8, high=.8)
  ...
  def calc_state(self):
    ...
    return np.array([x, vx, np.cos(self.theta), np.sin(self.theta), theta_dot]).astype(np.float32)
...
class InvertedDoublePendulum(MJCFBasedRobot):
  ...
  def __init__(self,randomExplor=True):
    MJCFBasedRobot.__init__(self, 'inverted_double_pendulum.xml', 'cart', action_dim=1, obs_dim=9,randomExplor=randomExplor)
  ...
  def robot_specific_reset(self, bullet_client):
    ...
    if self.randomExplor:
      u = self.np_random.uniform(low=-.2, high=.2, size=[2])#(low=-0.1, high=0.1)
