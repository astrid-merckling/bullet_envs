...
def get_fields(weight_save_name):
    ...
    if not match:
        raise error.Error(
            "Attempted to read a malformed weight save: {}. (Currently all weight saves must be of the form {}.)".format(
                id, weight_save_re.pattern
            )
        )
...
def get_latest_save(file_folder, agent_name, env_name, version_number):
    ...
    for f in file_list:
        ...
        if (
            file_properties[0] == agent_name
            and file_properties[1] == env_name
            and (
                latest_file_properties == []
                or file_properties[2] > latest_file_properties[2]
            )
        ):
