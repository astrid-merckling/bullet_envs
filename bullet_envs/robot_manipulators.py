...
class Reacher(MJCFBasedRobot):
    ...
    def init_goal(self):
        ...
        radius = self.np_random.uniform(low=0.05, high=self.TARG_LIMIT)
    ...
    def robot_specific_reset(self, bullet_client):
        ...
        if self.randomExplor:
            ...
            self.elbow_joint.reset_current_position(
                self.np_random.uniform(low=-2.65, high=2.65), 0
            )
        else:
        ...
        if self.distractor:
            ...
            radius = np.random.uniform(0.05, self.TARG_LIMIT)
            self.jdict["distractor_x"].reset_current_position(
                radius * np.cos(angle), np.random.uniform(-0.5, 0.5)
            )
            self.jdict["distractor_y"].reset_current_position(
...
class Pusher(MJCFBasedRobot):
    ...
    def robot_specific_reset(self, bullet_client):
        ...
        self.object_pos = (
            ...
            * self.np_random.uniform(
                low=self.min_object_to_target_distance,
                high=self.max_object_to_target_distance,
                size=1,
            )
        )
        ...
        self.jdict["target_x"].reset_current_position(
            self.target_pos[0] - self.zero_offset[0], 0
        )
        self.jdict["target_y"].reset_current_position(
            self.target_pos[1] - self.zero_offset[1], 0
        )
    ...
    def calc_state(self):
        ...
        return np.concatenate(
            [
                np.array(
                    [j.current_position() for j in self.ordered_joints]
...
class Thrower(MJCFBasedRobot):
    ...
    def robot_specific_reset(self, bullet_client):
        ...
        # reset object position
        self.parts["object"].reset_pose(
            self.object_pos - self.zero_offset, np.array([0, 0, 0, 1])
        )
